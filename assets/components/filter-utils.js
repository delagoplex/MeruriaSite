...
function useDragScroll({
  ...
  const fadeStyle = useMemo(() => {
    ...
    let g;
    if (vertical) {
      if (!atStart && !atEnd) g = `linear-gradient(to bottom, transparent, black ${px}px, black calc(100% - ${px}px), transparent)`;else if (!atStart) g = `linear-gradient(to bottom, transparent, black ${px}px)`;else if (!atEnd) g = `linear-gradient(to bottom, black calc(100% - ${px}px), transparent)`;
    } else if (horizontal) {
      if (!atStart && !atEnd) g = `linear-gradient(to right, transparent, black ${px}px, black calc(100% - ${px}px), transparent)`;else if (!atStart) g = `linear-gradient(to right, transparent, black ${px}px)`;else if (!atEnd) g = `linear-gradient(to right, black calc(100% - ${px}px), transparent)`;
    }
    return g ? {
      maskImage: g,
      WebkitMaskImage: g
...
function XBtn({
  ...
  return /*#__PURE__*/React.createElement("button", {
    ...
    onMouseEnter: e => {
      ...
      e.currentTarget.style.background = 'rgba(200,60,60,0.35)';
      e.currentTarget.style.borderColor = 'rgba(230,90,90,0.8)';
    },
    onMouseLeave: e => {
      e.currentTarget.style.color = 'rgba(230,90,90,0.9)';
...
function FilterGroup({
  ...
  }, collapsible && /*#__PURE__*/React.createElement("svg", {
    width: "8",
    height: "8",
    viewBox: "0 0 8 8",
    style: {
  ...
  }, title)), active && onReset && /*#__PURE__*/React.createElement(XBtn, {
    onClick: e => {
      e.stopPropagation();
      onReset();
    }
  })), (!collapsible || open) && /*#__PURE__*/React.createElement("div", {
    style: {
      animation: collapsible ? 'fadeIn 0.15s ease' : 'none'
    }
  }, children));
}
