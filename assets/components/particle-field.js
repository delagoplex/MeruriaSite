  ...
  useEffect(() => {
    ...
    const tick = now => {
      ...
      const w = canvas.width,
        h = canvas.height;
      const shiftX = (mouse.current.x - 0.5) * -14,
        shiftY = (mouse.current.y - 0.5) * -14;
      ...
      particles.current.forEach(p => {
        ...
        ctx.fillStyle = `rgba(${accentR},${accentG},${accentB},${p.alpha})`;
        ctx.fill();
      });
    };
    animRef.current = requestAnimationFrame(tick);
...
function ParticleFieldEnhanced({
  mouseX,
  mouseY
}) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      clearTimeout(resizeTimer);
      resizeTimer = setTimeout(() => {
        bg.width = fg.width = bg.offsetWidth;
        bg.height = fg.height = bg.offsetHeight;
      }, 150);
    };
    bg.width = fg.width = bg.offsetWidth;
    bg.height = fg.height = bg.offsetHeight;
    window.addEventListener('resize', resize);
    bgP.current = Array.from({
      length: 55
    }, () => ({
      x: Math.random(),
      y: Math.random(),
    ...
    const tick = now => {
      ...
      bgP.current.forEach(p => {
        ...
        bgCtx.fillStyle = `rgba(var(--accent-rgb),${p.alpha})`;
        bgCtx.fill();
      });
      ...
      fgP.current.forEach(p => {
        ...
        fgCtx.beginPath();
        fgCtx.arc(px, py, p.r * 2.5, 0, Math.PI * 2);
        fgCtx.fillStyle = g;
        fgCtx.fill();
        fgCtx.beginPath();
        fgCtx.arc(px, py, p.r, 0, Math.PI * 2);
        fgCtx.fillStyle = `rgba(var(--text-rgb),${p.alpha * 1.3})`;
        fgCtx.fill();
      });
    };
    animRef.current = requestAnimationFrame(tick);
