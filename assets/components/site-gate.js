...
function LoginForm({
  ...
  async function handleSubmit(e) {
    ...
    const {
      data: profile
    } = await window._sb.from('profiles').select('role').eq('id', data.user.id).single();
    window.SITE_USER = {
      id: data.user.id,
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Meruria")), /*#__PURE__*/React.createElement("form", {
    ...
    style: {
      animation: shake ? 'pulse-glow 0.4s ease' : 'none',
      display: 'flex',
      flexDirection: 'column',
      gap: '8px'
    }
    ...
    onFocus: e => {
      if (!error) e.target.style.borderColor = 'rgba(80,160,220,0.55)';
    },
    onBlur: e => {
      if (!error) e.target.style.borderColor = 'rgba(80,160,220,0.25)';
    }
    ...
    onMouseEnter: e => {
      if (!loading) {
        e.currentTarget.style.background = 'rgba(80,160,220,0.2)';
        e.currentTarget.style.borderColor = 'rgba(80,160,220,0.6)';
        e.currentTarget.style.color = 'var(--white)';
      }
    },
    onMouseLeave: e => {
      e.currentTarget.style.background = 'rgba(80,160,220,0.1)';
      e.currentTarget.style.borderColor = 'rgba(80,160,220,0.3)';
      e.currentTarget.style.color = 'rgba(150,200,240,0.75)';
    }
  }, loading ? '…' : 'Eintreten'))));
}
function SiteGate({
  ...
  React.useEffect(() => {
    async function init() {
      ...
      if (session) {
        const {
          data: profile
        } = await window._sb.from('profiles').select('role').eq('id', session.user.id).single();
        window.SITE_USER = {
          id: session.user.id,
  ...
  if (!authed) return /*#__PURE__*/React.createElement(LoginForm, {
